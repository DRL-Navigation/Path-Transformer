...
void Agent::set_sub_goal(const Point3d &pose)
{
    sub_target_pose_.x = pose.x;
    sub_target_pose_.y = pose.y;
    sub_target_pose_.z = robot_pose_.z;
    tf_sub_target_world_.setOrigin(tf::Vector3(sub_target_pose_.x, sub_target_pose_.y, 0));
    tf::Quaternion q;
    q.setRPY(0, 0, sub_target_pose_.z);
    tf_sub_target_world_.setRotation(q);
    tf_world_sub_target_ = tf_sub_target_world_.inverse();
}
...
void Agent::get_sub_goal(vector<double> &sub_goal)
{
    sub_goal.clear();
    tf::Transform tf_sub_target_base = (tf_world_sub_target_ * get_base_world()).inverse();
    tf::Vector3 t = tf_sub_target_base.getOrigin();
    tf::Quaternion q = tf_sub_target_base.getRotation();
    double roll, pitch, yaw;
    tf::Matrix3x3(q).getRPY(roll, pitch, yaw);
    sub_goal.push_back(t.x());
    sub_goal.push_back(t.y());
    sub_goal.push_back(yaw);
}
...
bool Agent::cmd(double v, double w,double v_y)
{
    ...
    double cur_dist = sqrt((robot_pose_.x - sub_target_pose_.x) * (robot_pose_.x - sub_target_pose_.x)
                                   + (robot_pose_.y - sub_target_pose_.y) * (robot_pose_.y - sub_target_pose_.y));
    if (cur_dist <= 0.3)
        {
            if (cur_traj_index_ == trajectory_.size()-1)
            {
                is_arrive = true;
            }
            else{
                is_arrive = false;
                cur_traj_index_ += 1;
                Point3d cur_rob_goal(trajectory_[cur_traj_index_].x, trajectory_[cur_traj_index_].y, 0);
                set_sub_goal(cur_rob_goal);
            }
        }
