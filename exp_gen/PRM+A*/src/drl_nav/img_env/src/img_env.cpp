...
//        eps_res_msg.robots_res[i].info.size = robots_[i].sizes_;
        ...
        robots_[i].trajectory_ = robot_msg.trajectory;
        robots_[i].cur_traj_index_ = 0;
        if( robots_[i].trajectory_.size() == 0 ){
            robots_[i].trajectory_.push_back(robot_msg.goal);
        }
        Point3d cur_rob_goal(robots_[i].trajectory_[0].x, robots_[i].trajectory_[0].y, 0);
        robots_[i].set_sub_goal(cur_rob_goal);
...
void ImgEnv::get_states(vector<comn_pkg::AgentState>& robot_states)
{
    for (int i = 0; i < robot_total_; i++)
    {
        ...
        vector<double> state;
        vector<double> sub_goal;
        robots_[i].get_state(state);
        robots_[i].get_sub_goal(sub_goal);
        state_msg.state.assign(state.begin(), state.end());
        state_msg.sub_goal.assign(sub_goal.begin(), sub_goal.end());
