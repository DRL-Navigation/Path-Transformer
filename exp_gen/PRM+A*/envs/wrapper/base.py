...
class TestEpisodeWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        self.a_sum += abs(speeds[0]-self.last_v)/self.dt
        self.last_v = speeds[0]
        return states, reward, done, info
    ...
    def reset(self, **kwargs):
        ...
        if self.cur_episode == self.max_episodes:
            ...
            """.format(self.max_episodes,
                       ...
                       self.a_sum / self.speed_step
                       ), flush=True)

        return self.env.reset(**kwargs)

class ExpPoolWrapper(gym.Wrapper):
    def __init__(self, env, cfg):
        super(ExpPoolWrapper, self).__init__(env)
        self.exp_pool = plyvel.DB("./output/exp_pool", create_if_missing=True)
        self.num = 0
    ...
    def reset(self, **kwargs):
        if self.finish == False:
            _len = int(str(self.exp_pool.get("Len".encode('UTF-8'), "0".encode('UTF-8')), encoding='UTF-8'))
            if self.sum > self.num:
                if self.last_reward > 0:
                    self.exp_pool.put("{}".format(_len).encode('UTF-8'), ExpPoolWrapper.__encode_data(self.exp))
                    self.exp_pool.put("Len".encode('UTF-8'), '{}'.format(_len+1).encode('UTF-8'))
                    self.num += 1
                    if self.num % 10000 == 0 and self.num != 0:
                        print("[{}]Got {}W Exp .".format(time.strftime('%H:%M:%S',time.localtime(time.time())), int(self.num/10000)), flush=True)
            else:
                self.exp_pool.close()
                self.finish = True
                print("[{}]Finish Exp Generate .".format(time.strftime('%H:%M:%S',time.localtime(time.time()))), flush=True)
                sys.exit(0)
        states = self.env.reset(**kwargs)
        self.exp = []
        self.exp.extend(states)
        self.last_reward = 0
        return states
    ...
    def step(self, action):
        states, reward, done, info = self.env.step(action)
        if self.finish == False:
            self.exp.append(reward.reshape(reward.shape+(1,)))
