...
class ImageEnv(gym.Env):
    ...
    def _init_static_param(self, cfg):
        ...
        # global_map_size
        tmp_map_png_shape = imageio.imread(get_map_file(self.map_file)).shape
        global_map_shape = (tmp_map_png_shape[0] * cfg['global_map']['resolution'], tmp_map_png_shape[1] * cfg['global_map']['resolution'])
        if cfg.get("global_map_size"):
            assert global_map_shape[0] == cfg.get("global_map_size")[0] and global_map_shape[1] == cfg.get("global_map_size")[1]
        else:
            cfg['global_map_size'] = global_map_shape
        self.global_map_size = global_map_shape
    ...
    def step(self, actions: List[ContinuousAction]):
        ...
        return state, rewards, deepcopy(self.dones), {'dones_info': np.zeros_like(self.dones), 'sub_goal':state.sub_goal}
    ...
    def _get_states(self, robot_states):
        vec_states, sub_goal, sensor_maps, lasers, ped_infos, ped_maps, is_collisions, is_arrives, distances = [], [], [], [], [], [], [], [], []
        ...
        for i in range(self.robot_total):
            ...
            sub_goal.append(robot_state.sub_goal)
