...
void ImgEnv::get_states(vector<comn_pkg::AgentState>& robot_states)
{
    for (int i = 0; i < robot_total_; i++)
    {
        ...
        state_msg.traj_collision = robots_[i].traj_collision_;
    ...
    // TODO: using peds_map_;
        ...
        for (int j = 0; j < robot_total_; j++)
        {
            if (i != j){
                ...
                robots_[j].get_traj_bbox(view_resolution_);
                robots_[j].check_traj(robots_[i].global_map_, robots_[j].traj_bbox_);
            }
        }
