...
void Agent::get_traj_bbox(double view_resolution_)
{
    vector<geometry_msgs::Point> traj = trajectory;
    geometry_msgs:: Point p;
    p.x = p.y = p.z = 0.0;
    traj.insert(traj.begin(), p);
    traj_bbox_.clear();
    for (int p=0; p<traj.size()-1; p++){
        double delta_x = traj[p+1].x - traj[p].x;
        double delta_y = traj[p+1].y - traj[p].y;
        double delta = delta_x*delta_x+delta_y*delta_y;
        double step_x = view_resolution_/delta*delta_x;
        double step_y = view_resolution_/delta*delta_y;
        int times_total = delta/view_resolution_;
        for (int t=0; t<times_total; t++){
            traj_bbox_.push_back(Point2d(traj[p].x+step_x*t, traj[p].y+step_y*t));
            traj_bbox_.push_back(Point2d(traj[p].x+step_x*t-view_resolution_, traj[p].y+step_y*t));
            traj_bbox_.push_back(Point2d(traj[p].x+step_x*t+view_resolution_, traj[p].y+step_y*t));
            traj_bbox_.push_back(Point2d(traj[p].x+step_x*t, traj[p].y+step_y*t-view_resolution_));
            traj_bbox_.push_back(Point2d(traj[p].x+step_x*t, traj[p].y+step_y*t+view_resolution_));
        }
    }
}

int Agent::check_traj(GridMap &grid_map, vector<Point2d>& bbox)
{
    traj_collision_ = 0;
